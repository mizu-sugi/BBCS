...
<div class="container mx-auto px-4 py-8">
        ...
        @if (auth('members')->id() === $message->member_id)
            <div class="flex justify-end mb-8">
                <a href="{{ route('members.community-boards.edit', $message->id) }}" class="text-blue-500 hover:underline mr-4">編集</a>
                <form action="{{ route('members.community-boards.destroy', $message->id) }}" method="POST" onsubmit="return confirm('本当に削除しますか？');">
                    @csrf
                    @method('DELETE')
                ...
                <div class="p-4">
                    ...
                    @if (auth('members')->id() === $comment->member_id)
                        <div class="flex justify-end mt-2">
                            <a href="{{ route('members.community-boards.comments.edit', [$message->id, $comment->id]) }}" class="text-blue-500 hover:underline mr-4">編集</a>
                            <form action="{{ route('members.community-boards.comments.destroy', [$message->id, $comment->id]) }}" method="POST" onsubmit="return confirm('本当に削除しますか？');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 hover:underline">削除</button>
                            </form>
        ...
        <form action="{{ route('members.community-boards.comments.store', $message->id) }}" method="POST" class="mb-8">
            @csrf
            <div class="mb-4">
