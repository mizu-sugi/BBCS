    ...
    <section>
        <div class="mb-6 py-4 bg-white rounded">
            <div class="flex px-6 pb-4 border-b">
                <h2 class="text-xl font-bold">健康記録一覧</h2>
                <div class="ml-auto">
                    <a href="{{ route('members.health-records.create') }}" class="py-2 px-3 text-xs text-white font-semibold bg-indigo-500 rounded-md">新規記録</a>
                </div>
            </div>
                ...
                <table class="table-auto w-full">
                    <thead>
                        <tr class="text-xs text-gray-500 text-left">
                            <th class="font-medium text-center">体温</th>
                            <th class="font-medium">吐き気</th>
                            <th class="font-medium">倦怠感</th>
                            <th class="font-medium">痛み</th>
                            <th class="font-medium">食欲</th>
                            <th class="font-medium">しびれ</th>
                            <th class="font-medium">不安感</th>
                            <th class="font-medium">その他の症状</th>
                            <th class="font-medium">記録日時</th>
                    ...
                    <tbody>
                        @foreach($healthRecords as $record)
                            <tr class="{{ $loop->odd ? 'bg-gray-50' : '' }}">
                                <td class="px-4 py-3 text-center">{{ $record->temperature }}℃</td>
                                <td class="px-4 py-3">{{ $record->nausea_level }}</td>
                                <td class="px-4 py-3">{{ $record->fatigue_level }}</td>
                                <td class="px-4 py-3">{{ $record->pain_level }}</td>
                                <td class="px-4 py-3">{{ $record->appetite_level }}</td>
                                <td class="px-4 py-3">{{ $record->numbness_level }}</td>
                                <td class="px-4 py-3">{{ $record->anxiety_level }}</td>
                                <td class="px-4 py-3">{{ $record->memo }}</td>
                                <td class="px-4 py-3">{{ $record->created_at }}</td>
                                <td class="px-4 py-3">
                                    <div class="flex">
                                        <a class="mr-2" href="{{ route('members.health-records.edit', ['healthRecord' => $record->id]) }}">
                                        ...
                                        <form action="{{ route('members.health-records.destroy', ['healthRecord' => $record->id]) }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <button>
        ...
        <!-- ▼▼▼▼ページャー▼▼▼▼　-->
        {{ $healthRecords->onEachSide(2)->links() }}
        <!-- ▲▲▲▲ページャー▲▲▲▲　-->
